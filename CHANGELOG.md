...
### Fixed
- Codex plugin package가 설치 cache 안에서 self-contained 하도록 고쳤다. Codex는 marketplace checkout의 parent 파일이나 symlink 대상 파일을 plugin cache로 함께 복사하지 않기 때문에 `plugins/vox-ai`를 source로 쓰면서 `../.mcp.json` 또는 symlink에 의존하면 `vox` / `vox-docs` MCP 서버가 로드되지 않는다.
