...
| Owns | Does Not Own |
|------|--------------|
| 대시보드 UI 조작 방법 안내 | 프롬프트 작성/진단 로직 (→ vox-agents) |
| 화면 기반 가이드 (Chrome MCP 연동) | 도구 생성/관리 로직 (→ vox-tools) |
| 다른 vox 스킬의 UI 레퍼런스 역할 | 플로우 에이전트 노드 설계 (→ vox-flow) |

## Related Resources

### MCP Tools (vox)
- `list_agents`, `get_agent` — 에이전트 목록/상세 조회 (UI 안내 시 실제 데이터 확인용)
- `create_campaign`, `list_campaigns` — 캠페인 생성/목록
...
- `https://www.tryvox.co/dashboard/{organizationId}/bulk-calls` — 대량 발신
- `https://www.tryvox.co/dashboard/{organizationId}/call-logs` — 통화 기록
