...
- **요약 규칙** — 이 문서 전체를 복사하지 않는다. 아래 "프롬프트에 적용하는 두 가지 방법"을 바탕으로 **해당 유즈케이스에 맞는 규칙 3~5개**만 요약해 `[[expressivity_rules]]`에 채운다.
...
### 방법 A — 시스템 프롬프트에 "사용 규칙"을 박기

LLM이 스스로 상황에 맞게 태그를 출력하도록 말투 섹션에 지시.
...
- 숫자/날짜/확정 정보는 `<speed ratio="0.9"/>`로 한 박자 천천히 읽어주세요.
```

### 방법 B — 고정 발화(첫 메시지, 안내 템플릿 등)에 직접 삽입

```xml
<emotion value="content"/> 안녕하세요 {{고객명}}님, <speed ratio="0.9"/> 오늘은 무엇을 도와드릴까요?
```
...
3. **한 발화에 과도하게 쓰지 말기** — 여러 태그를 동시에 쌓으면 덜 안정적. **중요한 포인트에만** 붙인다.
4. **한국어 검증은 별도 필요** — Cartesia 공식 예시는 영어 기반이다. 한국어 발화에서 각 emotion/speed 값이 어떻게 들리는지 **첫 배포 전 반드시 테스트 콜**로 확인.
5. **voice UX 기본 규칙이 우선** — 표현력을 추가해도 "질문 1개, 짧게 말하기" 같은 `voice-ai-playbook.md` 규칙은 그대로 유지된다.
